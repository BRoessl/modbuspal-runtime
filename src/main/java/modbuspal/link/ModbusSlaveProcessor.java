...
public abstract class ModbusSlaveProcessor implements ModbusConst {
	/**
	 * reference on the project that hold all the information for the modbus slaves
	 * to simulate
	 */
	...
	/**
	 * The subclass will call this method in order to process the content of the PDU
	 * that has been received from the master. The reply is written in the same
	 * buffer where the request was transmitted.
	 * 
	 ...
	 * @return the size of the reply. if less than 1, modbuspal considers that there
	...
	protected int buildPDU(ModbusMasterRequest req, ModbusSlaveAddress slaveID, byte[] buffer,
			int offset/* , int pduLength */) {
		// record the request
		// ModbusPalRecorder.recordIncoming(slaveID,buffer,offset,pduLength);
		...
		if (mspp == null) {
			System.err.println("Unsupported function code " + functionCode);
			// int length = makeExceptionResponse(functionCode,XC_ILLEGAL_FUNCTION, buffer,
			// offset);
			// ModbusPalRecorder.recordOutgoing(slaveID,buffer,offset,length);
	...
	/**
	 * The subclass will call this method in order to process the content of the PDU
	 * that has been received from the slave.
	 * 
	 ...
	 * @return
	 */
	protected boolean processPDU(ModbusMasterRequest req, ModbusSlaveAddress slaveID, byte[] buffer, int offset,
			int pduLength) {
		// record the request
		// ModbusPalRecorder.recordIncoming(slaveID,buffer,offset,pduLength);
	...
	/**
	 * Builds a standard exception response given the specified arguments. The
	 * function code is OR-ed with 0x80.
	 * 
	 * @param exceptionCode the code that explicits the cause of the exception
	 * @param buffer        the buffer where to write the exception
	 * @param offset        the offset in the buffer where to start writing
