...
public interface ModbusLink {
	/**
	 * Starts the ModbusLink in order to operate as a slave simulator. Usually
	 * creates a thread that is waiting for incoming requests.
	 * 
	 * @param l the modbus link listener that will receive notifications for the
	 *          events of this modbus link.
	 * @throws IOException
	 */
	public void start(ModbusLinkListener l) throws IOException;

	/**
	 * Starts the ModbusLink in order to operate as a MODBUS master. Usually opens a
	 * client connection. Requests are then managed by the execute method.
	 * 
	 * @param l
	 * @throws IOException
	 */
