...
/**
 * This class represents an entry from a recorded MODBUS session. Basically, a
 * record is either in incoming MODBUS request, or an outgoind reply.
 * 
 * @author nnovic
 */
public class ModbusPalRecord {
	/**
	 * indicates that a record is incoming, issued from an external device toward
	 * modbuspal.
	 */
	public static final int RECORD_IN = 1;

	/**
	 * indicates that a record is outgoing, issued by modbuspal toward an external
	 * device.
	 */
	...
	/**
	 * Creates a new record, with the data provided in argument. The object will
	 * parse the String in order to extract the meaningful data.
	 * 
	...
	public ModbusPalRecord(String line) {
		...
		Pattern p = Pattern.compile(
				"^<(\\p{Alpha}+)\\p{Space}+timestamp=(\\p{Digit}+)\\p{Space}+slave=(\\p{Digit}+)>([0-9a-fA-F]+)</(\\p{Alpha}+)>$");
	 ...
	 * RECORD_IN, it is the slave id of the target of the request. for a record a
	 * type RECORD_OUT, it is the slave id of the slave that replied to the request.
	 * 
	...
	/**
	 * Returns the data contained in the record. for records of type RECORD_IN, the
	 * data is an incoming PDU containing a MODBUS request. for records of type
	 * RECORD_OUT, the data is an outgoing reply to a request.
	 * 
