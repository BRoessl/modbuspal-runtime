...
public final class ModbusSlave implements ModbusPalXML, ModbusConst {
	 ...
	 * 
	 * @param a slave address assigned to this ModbusSlave.
	 */
	...
	/**
	 * Creates a new instance of ModbusSlave, by loading the settings in the
	 * provided DOM node.
	 * 
	...
	/**
	 * return an array with all the instances of ModbusPduProcessor that are use by
	 * this slave. Note that an instance of ModubsPduProcessor may be used for more
	 * than one MODBUS function code, but this list returns no duplicates. Also,
	 * note that the instances returned are only those modified by the user, by
	 * means of scripts. It won't return the predefined standard functions, such
	...
	void clearFunctions() {
		...
		/*
		 * for(byte i : USER_DEFINED_FUNCTION_CODES) { setPduProcessor(i,null); }
		 */
	 ...
	 * 
	 * @param functionCode the function code
	 * @param mspp         the new ModbusPduProcessor for the specified function
	 *                     code.
	 * @return the old ModbusPduProcessor that was defined for the specified
	 *         function code.
	 */
	...
	/**
	 * Remove all pdu processors with the specified class name from this slave.
	 * 
	 * @param classname
	 */
	 ...
	 * 
	 * @return array of strings containing the classnames of the automations used by
	 *         this slave.
	 */
	...
	private void loadFunctions(InstantiableManager<ModbusPduProcessor> ffactory, Node node) {
		if (node == null) {
			return;
		}
	...
	private void saveHoldingRegisters(OutputStream out, boolean withBindings) throws IOException {
	...
	private void saveCoils(OutputStream out, boolean withBindings) throws IOException {
	...
	private void saveFunctions(OutputStream out) throws IOException {
	...
	/**
	 * Overwrites the settings and values of this slave with those defined in the
	 * specified DOM node. The existing settings for: slave id, enabled status,
	...
	/**
	 * Overwrites the settings and values of this slave with those defined in the
	 * specified DOM node. Functions and tuning settings won't be loaded.
	 * 
	 * @param node       the DOM node where the settings to load are stored
	 * @param importMode if true, the loading from the DOM will take care not to
	 *                   overwrite the existing values of: slave id, enabled status,
	 *                   name.
	 ...
	 * 
	 * @param mpp  the modbuspal project from which additional information can be
	 *             retrieved. if null, functions and tuning settings won't be
	 *             loaded.
	 * @param node the DOM node where the settings to load are stored
	 */
	 ...
	 * specified DOM node.
	 * 
	 * @param mpp        the modbuspal project from which additional information can
	 *                   be retrieved. if null, functions and tuning settings won't
	 *                   be loaded.
	 * @param node       the DOM node where the settings to load are stored
	 * @param importMode if true, the loading from the DOM will take care not to
	 *                   overwrite the existing values of: slave id, enabled status,
	...
	/**
	 * Saves the values and settings of this modbus slave into the provided output
	 * stream, using XML format.
	 * 
	 * @param out          the output stream
	 * @param withBindings if true, the description of the bindings will be written,
	 *                     too
	 * @throws IOException
	 */
	public final void save(OutputStream out, boolean withBindings) throws IOException {
	...
	private void loadFunctionInstance(InstantiableManager<ModbusPduProcessor> ffactory, Node node)
	...
	/**
	 * Gets a random duration (in milliseconds) within the boundaries defined by the
	 * "min reply delay" and "max reply delay" parameters. The duration will be
	 * applied between the moment ModbusPal receives a request and the moment the
	 * reply (if any) is sent
	 * 
	 * @return a duration in milliseconds, delay between a modbus request and the
	 *         reply.
	 */
	...
	private void saveTuning(OutputStream out) throws IOException {
	 ...
	 */
	public void setReplyDelay(long min, long max) throws IllegalArgumentException {
		if ((min < 0) || (max < min)) {
	...
	/**
	 * Sets up the rates of the errors that will be simulated for this slave. For
	 * now, only the "no reply" error is defined. Each rate is a float value between
	 * 0f and 1f. 0f means "no error", or 0% of error. 1f means "always in error",
	 * or 100% of error. 0.5f will generate errors randomly, grossly 50% of the
	 * time.
	 * 
	 * @param noReply the rate at which this slave will fail to reply to a modbus
	 *                request
	 * @throws IllegalArgumentException
	 */
	public void setErrorRates(float noReply) throws IllegalArgumentException {
