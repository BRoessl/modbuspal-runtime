...
public class ModbusCoils extends ModbusRegisters {
	...
	public int processPDU(byte functionCode, ModbusSlaveAddress slaveID, byte[] buffer, int offset,
		...
		switch (functionCode) {
		case FC_READ_COILS:
			return processReadMultipleCoilsRequest(functionCode, buffer, offset, createIfNotExist);
		case FC_READ_DISCRETE_INPUTS:
			return processReadMultipleCoilsRequest(functionCode, buffer, offset, createIfNotExist);
		case FC_WRITE_SINGLE_COIL:
			return processWriteSingleCoilRequest(functionCode, buffer, offset, createIfNotExist);
		case FC_WRITE_MULTIPLE_COILS:
			return processWriteMultipleCoilsRequest(functionCode, buffer, offset, createIfNotExist);
		}
		return -1;
	}
	...
	public int buildPDU(ModbusMasterRequest req, ModbusSlaveAddress slaveID, byte[] buffer, int offset,
		...
		case FC_READ_COILS:
			return buildReadMultipleCoilsRequest(req, buffer, offset, createIfNotExist);
		case FC_READ_DISCRETE_INPUTS:
			return buildReadMultipleCoilsRequest(req, buffer, offset, createIfNotExist);
		case FC_WRITE_SINGLE_COIL:
			return buildWriteSingleCoilRequest(req, buffer, offset, createIfNotExist);
		case FC_WRITE_MULTIPLE_COILS:
			return buildWriteMultipleCoilsRequest(req, buffer, offset, createIfNotExist);
		}
		return -1;
	}
	...
	public boolean processPDU(ModbusMasterRequest req, ModbusSlaveAddress slaveID, byte[] buffer, int offset,
		...
		switch (req.getFunctionCode()) {
		case FC_READ_COILS:
			return readMultipleCoilsReply(req, buffer, offset, createIfNotExist);
		case FC_READ_DISCRETE_INPUTS:
			return readMultipleCoilsReply(req, buffer, offset, createIfNotExist);
		case FC_WRITE_SINGLE_COIL:
			return writeSingleCoilReply(req, buffer, offset, createIfNotExist);
		case FC_WRITE_MULTIPLE_COILS:
			return writeMultipleCoilsReply(req, buffer, offset, createIfNotExist);
		}
		return false;
	}

	private int processWriteMultipleCoilsRequest(byte functionCode, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private int buildWriteMultipleCoilsRequest(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private int buildWriteSingleCoilRequest(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private int processReadMultipleCoilsRequest(byte functionCode, byte[] buffer, int offset,
			boolean createIfNotExist) {
		int startingAddress = ModbusTools.getUint16(buffer, offset + 1);
		int quantity = ModbusTools.getUint16(buffer, offset + 3);
	...
	private int buildReadMultipleCoilsRequest(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private boolean readMultipleCoilsReply(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
		int startingAddress = req.getReadAddress();
		int quantity = req.getReadQuantity();
	...
	private boolean writeMultipleCoilsReply(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
		throw new UnsupportedOperationException("Not supported yet."); // To change body of generated methods, choose
																		// Tools | Templates.
	}
