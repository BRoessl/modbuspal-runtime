...
public class ModbusRegisters implements ModbusPduProcessor, ModbusPalXML, ModbusConst {
	...
	/**
	 * index of the column displaying the address of the registers in the
	 * TableModel.
	 */
	...
	/**
	 * index of the column displaying the binding of the registers in the
	 * TableModel.
	 */
	...
	@Override
	public int processPDU(byte functionCode, ModbusSlaveAddress slaveID, byte[] buffer, int offset,
			boolean createIfNotExist) {
		switch (functionCode) {
		case FC_READ_HOLDING_REGISTERS:
			return processReadMultipleRegistersRequest(functionCode, buffer, offset, createIfNotExist);
		case FC_WRITE_SINGLE_REGISTER:
			return processWriteSingleRegisterRequest(functionCode, buffer, offset, createIfNotExist);
		case FC_WRITE_MULTIPLE_REGISTERS:
			return processWriteMultipleRegistersRequest(functionCode, buffer, offset, createIfNotExist);
		case FC_READ_WRITE_MULTIPLE_REGISTERS:
			return processReadWriteMultipleRegistersRequest(functionCode, buffer, offset, createIfNotExist);
		}
		return -1;
	}

	@Override
	public int buildPDU(ModbusMasterRequest req, ModbusSlaveAddress slaveID, byte[] buffer, int offset,
			boolean createIfNotExist) {
		switch (req.getFunctionCode()) {
		case FC_READ_HOLDING_REGISTERS:
			return buildReadMultipleRegistersRequest(req, buffer, offset, createIfNotExist);
		case FC_WRITE_SINGLE_REGISTER:
			return buildWriteSingleRegisterRequest(req, buffer, offset, createIfNotExist);
		case FC_WRITE_MULTIPLE_REGISTERS:
			return buildWriteMultipleRegistersRequest(req, buffer, offset, createIfNotExist);
		case FC_READ_WRITE_MULTIPLE_REGISTERS:
			return buildReadWriteMultipleRegistersRequest(req, buffer, offset, createIfNotExist);
		}
		return -1;
	}

	@Override
	public boolean processPDU(ModbusMasterRequest req, ModbusSlaveAddress slaveID, byte[] buffer, int offset,
			boolean createIfNotExist) {
		switch (req.getFunctionCode()) {
		case FC_READ_HOLDING_REGISTERS:
			return readMultipleRegistersReply(req, buffer, offset, createIfNotExist);
		case FC_WRITE_SINGLE_REGISTER:
			return writeSingleRegisterReply(req, buffer, offset, createIfNotExist);
		case FC_WRITE_MULTIPLE_REGISTERS:
			return writeMultipleRegistersReply(req, buffer, offset, createIfNotExist);
		case FC_READ_WRITE_MULTIPLE_REGISTERS:
			return readWriteMultipleRegistersReply(req, buffer, offset, createIfNotExist);
		}
		return false;
	}
	...
	private int processReadWriteMultipleRegistersRequest(byte functionCode, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private int buildReadWriteMultipleRegistersRequest(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private boolean readWriteMultipleRegistersReply(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
		int readStartingAddress = req.getReadAddress();
		int quantityToRead = req.getReadQuantity();
		...
		/*
		 * // then perform read operation: buffer[offset+1] = (byte) (2*quantityToRead);
		 * for(int i=0; i<quantityToRead; i++) { Integer reg =
		 * getValue(readStartingAddress+i); ModbusTools.setUint16(buffer,
		 * offset+2+(2*i), reg); }
		 */
	...
	private int processWriteMultipleRegistersRequest(byte functionCode, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private int buildWriteMultipleRegistersRequest(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private boolean writeMultipleRegistersReply(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
		int startingAddress = ModbusTools.getUint16(buffer, offset + 1);
		int quantity = ModbusTools.getUint16(buffer, offset + 3);
		...
		/*
		 * byte rc = setValues(startingAddress, quantity, buffer, offset+6); if( rc !=
		 * XC_SUCCESSFUL ) { return
		 * ModbusSlaveProcessor.makeExceptionResponse(functionCode, rc, buffer, offset);
		 * }
		 * 
		 * return 5;
		 */
	...
	private int processWriteSingleRegisterRequest(byte functionCode, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private int buildWriteSingleRegisterRequest(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private boolean writeSingleRegisterReply(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
		int address = req.getWriteAddress();
		...
		/*
		 * Integer reg = ModbusTools.getUint16(buffer, offset + 3); byte rc =
		 * setValue(address,reg); if( rc != XC_SUCCESSFUL ) { return
		 * ModbusSlaveProcessor.makeExceptionResponse(functionCode, rc, buffer, offset);
		 * }
		 * 
		 * return 5;
	...
	private int processReadMultipleRegistersRequest(byte functionCode, byte[] buffer, int offset,
			boolean createIfNotExist) {
		int startingAddress = ModbusTools.getUint16(buffer, offset + 1);
		int quantity = ModbusTools.getUint16(buffer, offset + 3);
	...
	private int buildReadMultipleRegistersRequest(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	private boolean readMultipleRegistersReply(ModbusMasterRequest req, byte[] buffer, int offset,
			boolean createIfNotExist) {
	...
	/**
	 * Returns the value of the register, whose address is provided in argument.
	 * Note that depending on the implementation (modbus/jbus), an offset is applied
	 * to address. After the offset has been applied, returns the same result as
	 * getValue()
	 * 
	...
	/**
	 * Returns the value of the register, whose index is provided in argument. Note
	 * that index always starts from 0, with no consideration for the implementation
	 * offset (modbus/jbus). If the register is bound to an automation, returns the
	 * current value of the automation.
	 * 
	 * @param index the index of the register, starting from 0
	 * @return value of the register. If the register doesn't exist, returns 0 by
	 *         default.
	 */
	 ...
	 * indexes start from 0, whatever the slave's modbus implementation is
	 * (modbus,jbus,...). If the register is bound to an automation, this method has
	 * no effect on the current value of the automation. If index does not exist, an
	 * error code is returned and no action is performed. This method does not
	 ...
	 * depending on the implementation (modbus/jbus), an offset is applied to
	 * address. After the offset has been applied, returns the same result as
	 * setValue()
	 * 
	 * @param address the address of the register
	 ...
	 * (modbus,jbus,...). If the register is bound to an automation, this method has
	 * no effect on the current value of the automation. If index does not exist, an
	 * error code is returned and no action is performed. This method will trigger a
	 * TableEvent event, so that the GUI is refreshed.
	 * 
	 * @param index
	 * @param val
	 * @return the modbus error code indicating the success of the failure of the
	 *         action. In case of success, the returned value is XC_SUCCESSFUL
	 *         (0x00).
	 */
	...
	/**
	 * Reads the content of the byte buffer, which contains a range of values to
	 * update
	 * 
	...
	/**
	 * Convert the index of a value into the index of the row where this value is
	 * displayed in the TableModel.
	 * 
	...
	/**
	 * Checks if the given values are defined, but do not create them if they don't
	 * exist.
	 * 
	...
	/**
	 * Check if a range of registers, defined by the starting index and the quantity
	 * of registers in the range, is already defined or not. This method does not
	 * take the selected implementation into account. The first value is always at
	 * index 0.
	 * 
	...
	/**
	 * Check if a particular register, defined by its index, is already defined or
	 * not.
	 * 
	 * @param index
	 * @return true if the register is already defined, false otherwise
	 */
	...
	/**
	 * Removes a value from this object. The index is not sensitive to the selected
	 * implementation. The first index is always 0.
	 * 
	...
	/**
	 * Associates the register identified by its index with the provided binding.
	 * Note that indexes start from 0, whatever the slave's modbus implementation is
	 * (modbus,jbus,...). If a binding already exists, it is replaced by the new.
	 * 
	 * @param index
	 * @param binding
	 */
	 ...
	 * 
	 * @return true if any of the registers has a binding defined, false if none of
	 *         the register has a binding.
	 */
	 ...
	 * register, the allowed range is [0-65535]. For a coil, it is [0-1].
	 * 
	...
	/**
	 * Gets the name that has been defined for the specified value, identified by
	 * its index.
	 * 
	 * @param index index of the value
	...
	/**
	 * Gets a list of all the automations that are being used by this object,
	 * through the declared bindings.
	 * 
	 ...
	 * @param withBindings if true, the bindings of each value are also described in
	 *                     the output stream.
	 * @throws IOException
	 */
	public void save(OutputStream out, boolean withBindings) throws IOException {
	 ...
	 * event specifically addresses the row where the modified value is displayed.
	 * 
