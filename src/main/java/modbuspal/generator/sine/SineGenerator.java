...
public class SineGenerator extends Generator {
	...
	/*
	 * private void loadEnd(Node node) { // read attributes from xml document String
	 * endVal = XMLTools.getAttribute("value", node); String endRel =
	 * XMLTools.getAttribute("relative", node);
	 * 
	 * // setup generator's values endValue = Double.parseDouble(endVal);
	 * relativeEnd = Boolean.parseBoolean(endRel);
	 * 
	 * // update generator's panel panel.endTextField.setText(
	 * String.valueOf(endVal) ); panel.endRelativeCheckBox.setSelected(relativeEnd);
	 * }
	 * 
	 * private void loadStart(Node node) { // read attributes from xml document
	 * String startVal = XMLTools.getAttribute("value", node); String startRel =
	 * XMLTools.getAttribute("relative", node);
	 * 
	 * // setup generator's values startValue = Double.parseDouble(startVal);
	 * relativeStart = Boolean.parseBoolean(startRel);
	 * 
	 * // update generator's panel panel.startTextField.setText(
